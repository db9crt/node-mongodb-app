...
resource "azurerm_key_vault" "keyvaultprojns" {
  name                        = "keyvaultprojns"
...
resource "azurerm_key_vault_access_policy" "project-principalkey" {
  key_vault_id = azurerm_key_vault.keyvaultprojns.id
  tenant_id    = data.azurerm_client_config.current.tenant_id
...
resource "azurerm_windows_web_app" "MyNodeJsApp" {
  ...
  app_settings = {
    ...
    "DATABASE_URL" ="@Microsoft.KeyVault(SecretUri=${azurerm_key_vault.keyvaultprojns.vault_uri}secrets/${azurerm_key_vault_secret.projectsecretnewer.name}/${azurerm_key_vault_secret.projectsecretnewer.version})"  //"@Microsoft.KeyVault(SecretUri=https://keyvaultprojnsectdbstring.vault.azure.net/secrets/projectsecret)"//azurerm_cosmosdb_account.projectcosmosdbacct.connection_strings[0]  
  }
  identity {
    type = "SystemAssigned"
  }
